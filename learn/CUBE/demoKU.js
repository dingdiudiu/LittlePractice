...
DK.changeClassName = function(className, a, b) {
	if(className.indexOf(a) > 0){
		var rPattern = new RegExp("\\b" + a + "\\b");
		return className.replace(rPattern, b);
	}
	else if(className.indexOf(b) > 0){
		var rPattern = new RegExp("\\b" + b + "\\b");
		return className.replace(rPattern, a);
	}
	else{
		return className;
	}
